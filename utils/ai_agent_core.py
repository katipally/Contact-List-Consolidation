#!/usr/bin/env python3
"""
AI Brain for Contact Processing

This is the central AI system that makes smart decisions throughout the pipeline.
It talks to Ollama (the AI engine) to get intelligent answers about your contacts.

What it does:
- Figures out which contacts are duplicates by looking at all the details
- Helps map column names when your spreadsheet headers are different
- Extracts emails from web pages when we're searching for missing contact info
- Makes confident decisions and explains its reasoning

Think of it as the "smart assistant" that handles all the tricky decisions
...
class AIAgentCore:
    """
    The main AI brain that connects to Ollama
    This handles all the smart thinking for the contact processing pipeline
    """
    ...
    def __init__(self, ollama_url: str = "http://localhost:11434", model: str = "gemma3:4b", max_retries: int = 3):
        ...
        # Keep track of how well the AI is working
        self.stats = {
            "requests_made": 0,         # How many questions we asked the AI
            "successful_requests": 0,   # How many times it gave us good answers
            "failed_requests": 0,       # How many times it had problems
            "total_processing_time": 0.0, # Total time spent thinking
            "average_response_time": 0.0, # Average time per question
        }
