#!/usr/bin/env python3
"""
Email Finder (Agent 5)

This agent finds missing email addresses by intelligently searching the web.
It takes the clean contact list and tries to find emails for contacts that don't have them.

How it works:
1. Looks at each contact that's missing an email address
2. Creates smart search queries (like "John Smith Apple Inc email")
3. Searches the web using DuckDuckGo
4. Visits the most relevant websites
5. Uses AI to extract emails from the web pages
6. Adds the found emails back to your contact list

Features:
- Works on multiple contacts at the same time (faster)
- Skips contacts that already have emails
- Uses smart techniques to avoid getting blocked by websites
- Only finds professional/business emails (filters out junk)
- Handles errors gracefully and keeps trying
"""
